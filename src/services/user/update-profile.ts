...
export async function updateProfileImage(formData: FormData): Promise<any> {
    const cookieStore = await cookies();
    const accessToken = cookieStore.get("accessToken")?.value;
    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/users/update-profile-image`, {
            method: "PUT",
            headers: {
                Authorization: accessToken,
            },
        ...
        return {
            success: false,
            message: "Something went wrong while updating your profile image.",
        };
    }
}
